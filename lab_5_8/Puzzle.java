...
public class Puzzle {
    public static final int VALUE_0_FROM_8 = 9;
    ...
    public int computeH1(Node currentState) {
        ...
        for (int i = 0; i < goalState.getRow(); i++) {
            for (int j = 0; j < goalState.getColumn(); j++) {
                if (currentState.getTile(i, j) != goalState.getTile(i, j)) output += 1;
            }
        }
        return output;
    }
    ...
    public int computeH2(Node currentState) {
        int result = 0;
        /* Enter your code here */
        for (int i = 0; i < VALUE_0_FROM_8; i++) {
            int[] current = currentState.getLocation(i);
